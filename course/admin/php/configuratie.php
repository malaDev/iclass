	...
	<table id="course_overview">
		...
		while ($row = mysql_fetch_row($result)) {
			if (mysql_num_rows($result) != 0) {
				...
				<?php
				print '<form method="post" name="updateform' . $row[4] . '" action="'. rebase_path('admin/import').'">
							<tr><td><input class="inputfield" type="text" name="course_title" value="' . $row[0] . '" /></td>
								<td>' . $row[3] . ' door ' . $row[1] . ' ' . $row[2] . '</td><td><a class="save-link" title="veranderingen opslaan" onclick="document.updateform' . $row[4] . '.submit(); showLoading()">update</a><a class="delete-link" title="verwijderen" onclick="document.deleteform.submit();">delete</a></td>
							</tr>';
				?>
				<tr><td colspan="4"><b>XML-feed url (XBEL format): </b><input class="inputfield" onkeyup="checkEmpty()" type="text" name="course_xml" id="course_xml" value="<?php if (!preg_match("/^assets\/xml\/xml.txt/", $row[5]))
				echo $row[5]; ?>" /></td></tr>
				<tr><td colspan="4" id="course_xml_paste" <?php if (!preg_match("/^assets\/xml\/xml.txt/", $row[5]))
				echo 'style="display:none"'; ?>>of<br /><b>XML-feed plain text (XBEL format): </b><textarea name="course_xml_paste" style="border: 1px  #99d1f1 solid " cols="110" rows="30"><?php if (preg_match("/^assets\/xml\/xml.txt/", $row[5]))
				echo htmlspecialchars(file_get_contents($row[5])); ?></textarea></td>
					<td colspan="4" id="course_xml_paste_hidden_msg" <?php if (preg_match("/^assets\/xml\/xml.txt/", $row[5]))
				...
				?>
								<form name='deleteform' method='POST' action='<?php echo rebase_path('admin/import'); ?>'><input type='hidden' name='delete' value='<?php echo $row[4]; ?>' /></form>
		...
		if (mysql_num_rows($result) == 0) {
			?>
			<form method="post" name="createform" action="<?php echo rebase_path('admin/import'); ?>"><tr>
